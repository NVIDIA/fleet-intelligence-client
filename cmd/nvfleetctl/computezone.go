...
type computeZoneListOutput struct {
	ComputeZones []fleetintelligence.ComputeZone
	View         string
	JSONValue    any
	RawJSON      []byte
...
func newComputeZoneListCmd() *cobra.Command {
	flags := computeZoneListFlags{
		view: string(fleetintelligence.ComputeZoneViewDetail),
	}
...
func runComputeZoneList(cmd *cobra.Command, flags computeZoneListFlags, common resolvedCommonFlags) error {
	...
	opts := fleetintelligence.ListComputeZonesOptions{
		View:    fleetintelligence.ComputeZoneView(flags.view),
		ZoneIDs: zoneIDs,
	}
	applyPagination(common, func(page *int) { opts.Page = page }, func(pageSize *int) { opts.PageSize = pageSize })

	if common.all {
		var zones []fleetintelligence.ComputeZone
...
func validateComputeZoneListFlags(flags computeZoneListFlags, common resolvedCommonFlags) error {
	...
	if !fleetintelligence.ComputeZoneView(flags.view).Valid() {
...
func writeComputeZoneTable(w io.Writer, view string, zones []fleetintelligence.ComputeZone) error {
...
func basicComputeZoneRows(zones []fleetintelligence.ComputeZone) [][]string {
...
func detailComputeZoneRows(zones []fleetintelligence.ComputeZone) [][]string {
	rows := make([][]string, 0, len(zones))
	for _, zone := range zones {
