...
` + optionsHelpNote("nvfleetint xidburst options",
  ...
  nvfleetint xidburst list --window 168h --xid-numbers 48,94 --job-disruption`,
	...
	cmd.Flags().StringVar(&flags.xidNumbers, "xid-numbers", "", "XID numbers; a burst matches if it contains any of them")
	...
	cmd.Flags().StringVar(&flags.categories, "categories", "", "Exact category values to filter (cloud-provider/NCP only)")
	cmd.Flags().StringVar(&flags.subcategories, "subcategories", "", "Exact subcategory values to filter (cloud-provider/NCP only)")
	cmd.Flags().StringVar(&flags.tenantActions, "tenant-actions", "", "Exact tenant immediate-action codes to filter")
	cmd.Flags().StringVar(&flags.tenantInvestigations, "tenant-investigations", "", "Exact tenant investigatory-action codes to filter")
	cmd.Flags().StringVar(&flags.dcAdminActions, "dc-admin-actions", "", "Exact DC-admin immediate-action codes to filter (cloud-provider/NCP only)")
	cmd.Flags().StringVar(&flags.dcAdminInvestigations, "dc-admin-investigations", "", "Exact DC-admin investigatory-action codes to filter (cloud-provider/NCP only)")
