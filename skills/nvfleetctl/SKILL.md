...
Because a non-empty env var wins over the config file for *every* command, `nvfleetctl auth login` cannot fix a bad override: it writes to `~/.config/nvfleetctl/config.yaml`, but the stale `NVFLEETCTL_SERVICE_KEY` / `NVFLEETCTL_API_URL` keeps shadowing it. If `auth status` still fails after a login, or its `API URL:` line doesn't match what was logged in, have the user `unset NVFLEETCTL_SERVICE_KEY NVFLEETCTL_API_URL` in their shell (and remove it from any shell profile that exports it), then re-run `auth status`.
