...
// New builds an SDK client from the credentials selected by the common flags.
func New(common Resolved) (*nvfleetint.Client, error) {
	resolved, err := ResolveCredentials(common.Profile)
	if err != nil {
		return nil, err
	}

	return FromResolved(resolved, common)
}

// FromResolved builds an SDK client from already-resolved credentials, for a
// command that has resolved them itself.
func FromResolved(resolved config.Resolved, common Resolved) (*nvfleetint.Client, error) {
...
// ResolveCredentials resolves credentials, adding CLI hints to the config
// package's errors.
...
// MissingAPIKeyError explains which profile (if any) is missing an API key.
...
// FixAPIURLHint names the place a rejected API URL came from.
...
func commonClientOptions(common Resolved) []nvfleetint.Option {
