...
// OptionFlag describes how one backend filter field is spelled on the command
// line.
...
// OptionsPreamble states the two conventions once, so each section can be a bare flag name.
...
// OptionsHelpNote builds the command-level help note for flags whose values are discoverable
...
// OptionsRenderer renders a shared options envelope grouped by the flag that consumes each
...
// OptionSection groups the values one flag accepts under a heading that names it.
type OptionSection struct {
	Heading string
	Rows    [][]string
}

// Write renders the filter sections followed by the sorting flags.
...
// WriteOptionSection writes one filter section: a blank line, a heading naming the flag, then the
...
// ValueRows converts strings into single-column rows.
