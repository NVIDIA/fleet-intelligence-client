...
type alertTimelineNodesAllJSON struct {
	...
	Pagination               cmdutil.MergedPagination `json:"pagination"`
...
func newAlertListCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			return runAlertList(cmd, flags, cmdutil.ResolveCommon(cmd, common))
		},
	}
...
func newAlertSummaryCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Summarize impacted nodes and their alert counts.\n\n" +
			cmdutil.OptionsHelpNote("nvfleetint alert options",
				"--gpu-type", "--nodegroup-ids", "--compute-zone-ids",
				"--alert-state", "--component-type", "--sort-by", "--order"),
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			return runAlertSummary(cmd, flags, cmdutil.ResolveCommon(cmd, common))
		},
	}
...
func newAlertNodeCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return runAlertNode(cmd, args[0], flags, cmdutil.ResolveCommon(cmd, common))
		},
	}
...
func newAlertOptionsCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			return runAlertTimelineOptions(cmd, flags, cmdutil.ResolveCommon(cmd, common))
		},
	}
...
func newAlertDescribeCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Args:  cmdutil.RequireSingleArg("alert UUID"),
		RunE: func(cmd *cobra.Command, args []string) error {
			return runAlertDescribe(cmd, args[0], flags, cmdutil.ResolveCommon(cmd, common))
		},
	}
...
func runAlertList(cmd *cobra.Command, flags alertListFlags, common cmdutil.Resolved) error {
	...
	if err := opts.Validate(); err != nil {
		return cmdutil.RenderOptionError(err, alertListFlagNames)
	}
	...
	if common.All {
		...
		result, err := cmdutil.FetchAllPages("alerts",
...
func runAlertSummary(cmd *cobra.Command, flags alertTimelineFlags, common cmdutil.Resolved) error {
...
func runAlertNode(cmd *cobra.Command, nodeUUID string, flags alertTimelineFlags, common cmdutil.Resolved) error {
	active, err := resolveAlertView(flags.view)
	if err != nil {
		return err
	}
...
func runAlertTimelineOptions(cmd *cobra.Command, flags alertTimelineOptionsFlags, common cmdutil.Resolved) error {
...
func runAlertTimelineNodes(cmd *cobra.Command, client *nvfleetint.Client, opts nvfleetint.ListAlertTimelineNodesOptions, common cmdutil.Resolved) error {
	cmdutil.ApplyPagination(common, func(page *int) { opts.Page = page }, func(pageSize *int) { opts.PageSize = pageSize })
...
func runNodeAlertTimeline(cmd *cobra.Command, client *nvfleetint.Client, opts nvfleetint.ListNodeAlertTimelineOptions, common cmdutil.Resolved) error {
	cmdutil.ApplyPagination(common, func(page *int) { opts.Page = page }, func(pageSize *int) { opts.PageSize = pageSize })
...
func runAlertDescribe(cmd *cobra.Command, alertUUID string, flags alertDescribeFlags, common cmdutil.Resolved) error {
	...
	if common.Output == clioutput.FormatJSON {
		if opts.PageSize != nil {
			return clioutput.WriteRawJSON(cmd.OutOrStdout(), cmdutil.OneIndexRawPage(details.RawJSON))
		}
...
var (
	alertTimelineNodesFlagNames = map[string]cmdutil.OptionFlagName{
		"order":      {Flag: "order"},
	...
	nodeAlertTimelineFlagNames = map[string]cmdutil.OptionFlagName{
		"order":      {Flag: "order"},
		"alertState": {Flag: "alert-state"},
...
func alertTimelineFilters(flags alertTimelineFlags) (states []nvfleetint.AlertTimelineState, lists [4][]string, err error) {
	states, err = cmdutil.ParseTypedList[nvfleetint.AlertTimelineState]("alert-state", flags.alertState)
	if err != nil {
	...
	for index, filter := range [4]struct {
		...
		values, parseErr := cmdutil.ParseCommaList(filter.raw)
		if parseErr != nil {
...
func alertTimelineNodesOptions(flags alertTimelineFlags) (nvfleetint.ListAlertTimelineNodesOptions, error) {
	...
	if err := opts.Validate(); err != nil {
		return opts, cmdutil.RenderOptionError(err, alertTimelineNodesFlagNames)
	}
...
func nodeAlertTimelineOptions(flags alertTimelineFlags) (nvfleetint.ListNodeAlertTimelineOptions, error) {
	...
	if err := opts.Validate(); err != nil {
		return opts, cmdutil.RenderOptionError(err, nodeAlertTimelineFlagNames)
	}
...
func validateAlertDescribeFlags(cmd *cobra.Command, flags alertDescribeFlags, common cmdutil.Resolved) error {
	if err := cmdutil.ValidateReadFlags(common); err != nil {
		return err
	}
	...
	if cmd.Flags().Changed("page-size") && (flags.pageSize < cmdutil.MinPageSize || flags.pageSize > cmdutil.MaxPageSize) {
		return fmt.Errorf("--page-size must be between %d and %d", cmdutil.MinPageSize, cmdutil.MaxPageSize)
	}
	return nil
}
...
var alertListFlagNames = map[string]cmdutil.OptionFlagName{
	"state":    {Flag: "state"},
	"severity": {Flag: "severity"},
}

// Writes JSON or table output for alert list results
func writeAlertListOutput(w io.Writer, common cmdutil.Resolved, result alertListOutput) error {
	if common.Output == clioutput.FormatJSON {
...
func writeAlertTimelineOutput(w io.Writer, common cmdutil.Resolved, result alertTimelineOutput) error {
	if common.Output == clioutput.FormatJSON {
		return cmdutil.WritePaginatedListJSON(w, result.RawJSON, result.JSONValue)
	}
...
var alertOptionsRenderer = cmdutil.OptionsRenderer{
