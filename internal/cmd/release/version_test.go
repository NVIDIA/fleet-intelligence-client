...
// testBuild is the build the release commands under test report. The real
// values are stamped in at link time, which a test binary never gets. setVersion
// overrides the version for a single test.
var testBuild = BuildInfo{Version: "dev", Commit: "abc123", BuildDate: "2026-01-01"}
...
func newRootCmd() *cobra.Command {
	...
	root.AddCommand(NewVersionCmd(testBuild))
	root.AddCommand(NewUpgradeCmd(testBuild))
	return root
}
...
// Overrides the version the commands under test report as the running build.
func setVersion(t *testing.T, value string) {
	t.Helper()

	previous := testBuild.Version
	testBuild.Version = value
	t.Cleanup(func() { testBuild.Version = previous })
}
